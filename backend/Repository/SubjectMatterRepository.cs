...
public class SubjectMatterRepository : ISubjectMatterRepository
{
    ...
    {
        return await _context.Course.Include(c => c.CourseSubjectMatters
            .Where(csm => csm.SubjectMatterId == id))
        .AsNoTracking()
        .ToListAsync();
    }
    ...
    {
        return await _context.Student.Include(s => s.StudentSubjectMatters
            .Where(ssm => ssm.SubjectMatterId == id))
        .AsNoTracking()
        .ToListAsync();
    }
